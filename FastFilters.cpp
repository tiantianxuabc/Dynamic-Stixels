...
inline uint16* getPixel16(uint16* base, uint32 width, int j, int i)/*0000*/
{
	return base + i*width + j;
}

inline uint32* getPixel32(uint32* base, uint32 width, int j, int i)/*0000*/
{
	return base + i*width + j;
}
...
*dest  census结果
*/
void census5x5_16bit_SSE(uint16* source, uint32* dest, uint32 width, uint32 height)/*0000*/
{
	uint32* dst = dest;
...
inline void vecSortandSwap(__m128& a, __m128& b)/*0000*/
{
	__m128 temp = a;
	a = _mm_min_ps(a, b);
	b = _mm_max_ps(temp, b);
...
void median3x3_SSE(float32* source, float32* dest, uint32 width, uint32 height)/*0000*/
{
