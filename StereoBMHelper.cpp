...
/*计算视差空间dsi*/
void costMeasureCensus5x5Line_xyd_SSE(uint32* intermediate1, uint32* intermediate2 /*0000*/
	, const int width, const int dispCount, const uint16 invalidDispValue, uint16* dsi, const int lineStart, const int lineEnd)
{
	...
	for (sint32 i = lineStart; i < lineEnd; i++) {/*有效值呈阶梯状分布.[0]、[0,1]、[0,1,2]、[0,1,..,62]、[0,1,..,62,63]、...[0,1,..,63]*/
		...
		for (uint32 j = 0; j < (uint32)width; j++)
		{
			...
			//pBaseJ指向第i行的第j个的前63(rightImgCensus)---采样当前值和前63个值。
...
void costMeasureCensus5x5_xyd_SSE(uint32* intermediate1, uint32* intermediate2 /*0000*/
...
void matchWTA_SSE(float32* dispImg, uint16* &dsiAgg, const int width, const int height, const int maxDisp, const float32 uniqueness)/*0000*/
{
...
void matchWTARight_SSE(float32* dispImg, uint16* &dsiAgg, const int width, const int height, const int maxDisp, const float32 uniqueness)/*0000*/
{
	const uint32 factorUniq = (uint32)(1024 * uniqueness);
...
void subPixelRefine(float32* dispImg, uint16* dsiImg, const sint32 width, const sint32 height, const sint32 maxDisp)/*0000*/
{
